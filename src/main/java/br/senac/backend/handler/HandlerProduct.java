...
public class HandlerProduct {
    ...
    public void handleAddMessages(ResponseAPI response, int code, Object model) {
        if (code == 200) {
            ...
            response.setMsg("SUCCESSFULLY_REGISTERED_PRODUCT");
        } else if (code == 404) {
            response.setCode(404);
            response.setData(null);
            response.setMsg("PRODUCT_ADDED_NOT_FOUND");
        } else if (code == 304) {
            response.setCode(304);
            response.setMsg("PRODUCT_ALREADY_EXISTS");
            response.setData(null);
        } else if (code == 400) {
    ...
    public void handleUpdateMessages(ResponseAPI response, int code, Object model) {
        if (code == 200) {
            response.setCode(200);
            response.setData(model);
            response.setMsg("PRODUCT_UPDATED_SUCCESSFULLY");
        } else if (code == 404) {
            response.setCode(404);
            response.setData(null);
            response.setMsg("PRODUCT_UPDATED_NOT_FOUND");
        } else if (code == 304) {
            response.setCode(304);
            response.setMsg("PRODUCT_ALREADY_EXISTS");
            response.setData(null);
    ...
    public void handleDetailMessages(ResponseAPI response, int code, Object model) {
        if (code == 200) {
            response.setCode(200);
            response.setData(model);
            response.setMsg("PRODUCT_DETAIL_SUCCESSFULLY");
        } else if (code == 404) {
            response.setCode(404);
            response.setData(null);
            response.setMsg("PRODUCT_NOT_FOUND");
        } else if (code == 400) {
            response.setCode(400);
            response.setMsg("BAD_REQUEST");
            response.setData(null);
    ...
    public void handleDeleteMessages(ResponseAPI response, int code) {
        if (code == 200) {
            response.setCode(200);
            response.setData(null);
            response.setMsg("PRODUCT_DELETE_SUCCESSFULLY");
        } else if (code == 404) {
            response.setCode(404);
            response.setData(null);
            response.setMsg("PRODUCT_NOT_FOUND");
    ...
    public void handleListMessages(ResponseAPI response, int code, Object model) {
        if (code == 200) {
            response.setCode(200);
            response.setData(model);
            response.setMsg("PRODUCT_LIST_SUCCESSFULLY");
        } else if (code == 404) {
            response.setCode(404);
            response.setData(null);
            response.setMsg("PRODUCT_NOT_FOUND");
