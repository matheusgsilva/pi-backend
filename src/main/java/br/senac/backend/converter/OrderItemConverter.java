	...
	public OrderItemResponse orderItemToResponse(OrderItem orderItem) {
		try {
			...
			if (orderItem.getProduct().getStock() != null) {
				orderItemResponse.setQuantityAvailable(orderItem.getProduct().getStock().getQuantity());
				orderItemResponse.setWeightAvailable(orderItem.getProduct().getStock().getWeight());
			}
